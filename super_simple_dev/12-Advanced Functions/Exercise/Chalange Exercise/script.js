...
const essential = {
    ...
    autoPlayBT: document.querySelector('#autoPlay'),
    rstConfirmElement: document.querySelector('.resetConfirm')
};
...
function autoPlay(){
    if(!autoPlaying){
        intervalId = setInterval(function(){
            ...
            function1(playerMove);
        }, 1000)
        autoPlaying = true;
        essential.autoPlayBT.innerHTML = 'Turn off'
    }
    else{
        clearInterval(intervalId);
        autoPlaying = false;
        essential.autoPlayBT.innerHTML = 'Auto Play'
    }
}

const resetConfirm = ()=>{
    essential.rstConfirmElement.innerHTML = `
    <p class = "genConfMess">Do you want to delete game data ?</p>    
...
document.body.addEventListener('keydown', (e)=>{
    ...
    if(e.key === 'r' || e.key === 'R'){
        ...
        console.log('Valid keys to play the game are r/R, p/P, s/S')
    }
...
essential.autoPlayBT.addEventListener('click', function(){
    autoPlay();
})
